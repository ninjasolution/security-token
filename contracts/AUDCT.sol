...
contract AUDCT is ERC20, Ownable, Pausable {
    ...
    constructor(
        address _router,
        address _vault
    ) ERC20("Australian Decentralised Capital Token", "AUDCT") {
    ...
    function _transfer(
        ...
        if (msg.sender == owner()) {
            ...
            taxAmount = amount.mul(buyFee).div(divisor);
        } else if (to == address(uniswapV2Pair)) {
            taxAmount = amount.mul(sellFee).div(divisor);
        }
    ...
    /**
     * @dev update tax fee vault wallet.
     */
    function udpateVault(address _vault) external onlyOwner {
        vault = _vault;
     ...
     * @dev update tax fees.
     */
    function updateTaxFee(uint32 _buyFee, uint32 _sellFee) external onlyOwner {
        require(
            _buyFee <= maxBuyFee && _sellFee <= maxSellFee,
            "AUDCT: Exceeded max tax fee"
        );
        buyFee = _buyFee;
        sellFee = _sellFee;
    }
